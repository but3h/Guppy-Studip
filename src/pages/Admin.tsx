...
import { Plus, Trash2, Package, Image as ImageIcon, Tag, Banknote, ListChecks, X, CheckCircle2, AlertCircle, LayoutGrid, Box, Save, MinusCircle, PlusCircle, ShoppingCart, UserCircle, Phone, CreditCard, MapPin, Archive, CheckCircle, Bell, BellOff } from 'lucide-react';
...
export default function Admin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', description: '', price: '', category: 'guppy', photoUrl: '', maleStock: '10', femaleStock: '10', specs: '' });
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12">
          ...
          <div className="mt-4 p-3 bg-white/50 rounded-xl border border-gray-200 text-xs text-gray-500 w-fit">
        ...
        {/* Tab Navigation */}
        <div className="flex gap-4 mb-8 p-1.5 bg-gray-200/50 rounded-2xl w-fit">
          <button 
                    ...
                    {products.map(p => (
                      <motion.div 
                        ...
                        className={`flex flex-col sm:flex-row items-center gap-6 p-6 rounded-3xl border transition-all group ${
                          (p.maleStock === 0 && p.femaleStock === 0) ? 'bg-red-50/30 border-red-100' : (p.maleStock < 5 || p.femaleStock < 5) ? 'bg-orange-50/30 border-orange-100' : 'bg-gray-50 border-gray-100 hover:bg-white hover:shadow-lg'
                        }`}
                      >
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-6">
                  {orders
                    ...
                    .map(order => (
                    <div key={order.id} className="p-4 sm:p-6 rounded-3xl border border-gray-100 bg-gray-50/50 hover:bg-white hover:shadow-xl transition-all group">
                  ...
                  {orders.filter(order => {
                    ...
                    <div className="text-center py-20 bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200">
                      ...
                      <h3 className="text-lg font-bold text-gray-900 mb-1">
                        {orderView === 'active' ? 'No active orders' : orderView === 'delivered' ? 'No delivered orders' : 'No cancelled orders'}
                      </h3>
                      <p className="text-gray-500">
                        {orderView === 'active' 
                          ? 'When customers buy products, they will appear here.' 
                          : orderView === 'delivered'
                            ? 'Orders moved to delivered archive will appear here.'
                            : 'Cancelled orders moved to archive will appear here.'}
                      </p>
                    </div>
                  )}
                </div>
